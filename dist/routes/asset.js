 ...
 */
router.get("/:tokenId/owner", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        //@ts-ignore
        const owner = await contract.ownerOf(tokenId);
 ...
 */
router.get("/:tokenId/permissions/:user", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        ...
        //@ts-ignore
        const permission = await contract.getPermission(tokenId, user);
        return res.json({
            tokenId: tokenId.toString(),
            user,
 ...
 */
router.get("/:tokenId/permissions/:user/check", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        const user = req.params.user;
        const requiredPermission = Number(req.query.permission);
        if (!ethers.isAddress(user)) {
            return res.status(400).json({
                error: "Invalid Ethereum address"
            });
        }
        if (!Number.isInteger(requiredPermission) ||
        ...
        //@ts-ignore
        const allowed = await contract.hasPermission(tokenId, user, requiredPermission);
        return res.json({
            tokenId: tokenId.toString(),
            user,
            requiredPermission,
 ...
 */
router.post("/:tokenId/permissions", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        const { user, permission, expiresAt = 0 } = req.body;
        if (!ethers.isAddress(user)) {
            return res.status(400).json({
        ...
        if (!Number.isInteger(permissionValue) ||
            permissionValue < 1 ||
            permissionValue > 4) {
            return res.status(400).json({
                error: "permission must be between 1 and 4"
        ...
        //@ts-ignore
        const tx = await writeContract.grantPermission(tokenId, user, permissionValue, BigInt(expiresAt));
        const receipt = await tx.wait();
        return res.json({
            success: true,
            transactionHash: receipt.hash,
 ...
 */
router.delete("/:tokenId/permissions/:user", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        const user = req.params.user;
        if (!ethers.isAddress(user)) {
        ...
        //@ts-ignore
        const tx = await writeContract.revokePermission(tokenId, user);
        const receipt = await tx.wait();
        return res.json({
            success: true,
            transactionHash: receipt.hash,
...
router.get("/:tokenId/history", async (req, res) => {
    try {
        const tokenId = BigInt(req.params.tokenId);
        //@ts-ignore
        const length = await contract.getOwnershipHistoryLength(tokenId);
        const history = [];
        for (let i = 0n; i < length; i++) {
            //@ts-ignore
            const record = await contract.getOwnershipRecord(tokenId, i);
            history.push({
                index: i.toString(),
